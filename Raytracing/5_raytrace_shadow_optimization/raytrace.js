...
function recursive_ray(ray_source, ray_direction, depth) {
  ...
  if (depth > 0) {
    ...
    if (intersected) {
      ...
      var in_shadow = false;
      for (var obj of list_of_scene_objects) {
        // exclude nearest object
        if (obj != nearest_obj) {
          // check intersection along reflected ray
          var [hit_tmp, intersection_tmp, normal_tmp, rgb_tmp] = obj.intersection(
            nearest_intersection_point,
            to_light_source
          );
          if (hit_tmp) {
      ...
      // if in shadow, half the lighting factor
      if (in_shadow) {
        lighting *= 0.5;
      }

      // update rgb with lighting factor
      nearest_rgb = p5.Vector.mult(nearest_rgb, lighting);
